...
和 playground 的区别：playground 也走 `@snowbitx/*`，但那是开发主战场，样式外壳、代理层都是为「好用」服务；smoke 刻意保持最小（一个壳 + 12 页），**只验证一件事——发包形态能不能被正常 import 并运行**。

## 验证什么

- **主入口（barrel）**：`import { ProButton, ... } from '@snowbitx/ui-xxx'`
- **深路径**：`@snowbitx/ui-xxx/components/ProTable` 等 exports 里的每个子路径
